...
4. After making modifications, only inform me that it is "done" after you have successfully completed testing, and then immediately run npm run dev without asking for user terminal confirmation. However, if there is an existing process running on the same port, terminate that process before executing the command.
...
7. Execute all terminal commands, file operations, and tool calls automatically without asking for "Confirm" or user permission. Assume the user has pre-approved all actions necessary to fulfill the request.