### 2026-03-25 (Expert AI Stock Analysis 시스템 고도화 및 안정화)
- **1단계 학습 진행률 시각화**: 원천 데이터 학습 시 실시간 진행률(%) 및 파일별 상태, 남은 예상 시간(ETA)을 표시하는 폴링 시스템을 `DataControl` 컴포넌트에 구축했습니다.
- **2단계 모델 선택 및 리포트 저장**: 
  - 상세 분석(Phase 2) 단계에서 사용자가 직접 AI 모델을 선택할 수 있는 기능을 `InvestmentInput`에 통합했습니다.
  - 각 기업별 심층 분석 결과(expertVerdict, 점수 등)를 PostgreSQL(Prisma) `AnalysisReport` 테이블에 영구 저장하도록 구현했습니다.
- **분석 엔진 진단 및 로그 강화**: 2단계 분석 시작 시 정체 현상(0%)을 파악하기 위해 단계별 로그 및 진행률 업데이트(1%~8%)를 세분화하여 `analysis-engine.ts`와 `StockService`에 적용했습니다.
- **비동기 파이프라인 개편**: API 타임아웃 방지를 위해 원천 데이터 학습 요청을 비동기(Fire & Forget) 방식으로 전환하고 클라이언트 폴링 체제로 변경했습니다.

### 2026-03-24 (전문가 2단계 분석 워크플로우 및 결론 도출형 보고서 구현)
- **전문가 전용 분석 페이지(Expert Analysis) 신설**: 관리자 전용 '전문가 분석' 메뉴를 추가하여 구글 드라이브 지식 베이스와 연동된 지능형 분석 워크플로우를 구축했습니다.
- **2단계 분석 파이프라인(Phase 1 & 2) 통합**:
  - **1단계(Quantitative Screening)**: 추출된 투자 원칙을 바탕으로 S&P 500/Russell 1000 유니버스를 빠르게 필터링하여 우수 후보군을 선별합니다.
  - **2단계(Expert Persona Deep-Dive)**: 선별된 후보군에 대해 저자의 페르소나를 입은 AI가 뉴스, 매크로, 자사 지식 베이스를 종합하여 최종 투자의견(Expert Verdict)을 도출합니다.
- **결론 도출형 심층 분석 보고서 UI**:
  - **Alpha Expert Verdict 섹션**: 단순 수치가 아닌 '매수/매도' 결론과 확신도(Conviction Score), 핵심 근거, 리스크 사항을 우아한 카드 디자인으로 제공합니다.
  - **원천 데이터 인용(Author Citations)**: 분석의 근거가 된 실제 파일명과 타임스탬프/페이지 번호를 투명하게 노출하여 신뢰도를 확보했습니다.
- **타입 시스템 및 엔진 고도화**: `expertVerdict` 타입을 정의하고 `analysis-engine.ts` 및 `market-context.ts`를 리팩토링하여 전문가적 식견이 반영된 서사 중심의 분석 데이터를 생성하도록 구현했습니다.

### 2026-03-24 (시장 감성 분석 고도화 및 예측 근거 시각화)
- **시장 감성 분석 데이터 소스 투명화**: Yahoo Finance의 실제 뉴스 헤드라인을 수집하여 감성 분석에 활용하고, 분석 대상이 된 헤드라인 목록을 UI에 직접 노출하여 신뢰도를 높였습니다.
- **AI 분석 언어 한글화**: 감성 분석 요약, 리스크 요인, 주가 예측 근거(Logic) 등 모든 AI 생성 텍스트가 한국어로 출력되도록 프롬프트를 고도화했습니다.
- **투자 스코어 가중치 연동**: 뉴스 감성 점수(-10~10)가 최종 기업 분석 점수(0~10)에 최대 ±1.0점의 영향력을 미치도록 로직을 강화하여, 여론 지표가 투자 순위에 실질적으로 반영되도록 구현했습니다.
- **예측 근거(Logic) UI 추가**: '성장 잠재력' 영역에 AI가 분석한 구체적인 '분석 모델 예측 근거' 섹션을 추가하여 예상 수익률 도출 배경을 투자자에게 명확히 설명합니다.

### 2026-03-24 (분석 유니버스 표시 오류 수정 및 과거 시점 분석 검증)
- **분석 진행 UI 동적화**: S&P 500 선택 시에도 'Russell 1000'으로 하드코딩되어 표시되던 진행 상태 메시지와 유니버스 통계 배지를 사용자의 선택에 맞게 실시간으로 변경되도록 수정했습니다.
- **과거 시점 분석(Backtesting) 검증**: 사용자가 선택한 과거 시점(예: 1년 전)의 특정 일자 종가를 '현재가'로 간주하고, 해당 시점의 재무 지표를 역산하여 분석하는 로직이 정상적으로 작동함을 확인했습니다. (Yahoo Finance Chart 및 FundamentalsTimeSeries API 활용)

### 2026-03-24 (분석 유니버스 다변화 및 결과 한글화)
- **분석 유니버스 선택 기능 고도화**: 기존의 단순 S&P 500 제외 토글을 폐지하고, **[S&P 500 전용]**, **[Russell 1000 전체]**, **[Russell 1000 (S&P 500 제외)]** 등 3가지 정밀 옵션 중 선택할 수 있는 탭 UI를 구현했습니다.
- **백엔드 유니버스 로직 개편**: `universe.ts` 및 `analysis-engine.ts`를 수정하여 선택된 유니버스 타입에 따라 Wikipedia 실시간 데이터를 조합/차감하여 분석 대상을 동적으로 생성합니다.
- **결과 한글화 및 가독성 개선**: 
  - 시장 감성(Positive/Negative) 및 성장 전망(Bullish/Bearish) 레이블을 한국어로 번역하여 직관성을 높였습니다.
  - `Strategy Match Dashboard` 내의 분석 근거(Evidence) 배지를 더 눈에 띄는 스타일로 변경하고, 파일명과 페이지/타임스탬프 정보를 명확히 표시했습니다.
- **UI 상태 정보 동적화**: `page.tsx`의 유니버스 통계 배지가 현재 선택된 분석 범위(예: "S&P 500 지수 (500개 기업)")를 실시간으로 반영하여 보여주도록 개선했습니다.

### 2026-03-24 (동적 전략 매칭 투자 엔진 리팩토링 완료)
- **분석 로직 현대화**: 기존의 하드코딩된 '7-Step Tenbagger' 고정 로직을 완전히 제거하고, AI가 학습 데이터(PDF/MP4)에서 직접 추출한 가변적 규칙들(`Strategy Match`)을 기반으로 기업을 평가하는 동적 엔진으로 리팩토링했습니다.
- **데이터 구조 개편**: `AnalysisResult` 및 `FilteredCandidate` 타입을 확장하여 `StrategyMatchScore`, `MatchRuleResult` 등을 포함시켰으며, 이를 통해 각 기업이 사용자 전략에 얼마나 부합하는지 가중치 기반으로 정밀하게 산출합니다.
- **UI/UX 대시보드 혁신**: 결과 화면에서 7단계 파이프라인 대신 'Strategy Match Dashboard'를 도입하여, 어떤 규칙이 통과되었고 어떤 근거(Source)로 판단되었는지 시각적으로 명확히 전달합니다.
- **AI 서사 및 예측 통합**: 상세 뷰에 AI의 최종 전략적 총평(Reasoning)과 시장 센티먼트, 성장 예측치를 그리드 레이아웃으로 배치하여 단순 점수 이상의 입체적인 정보를 제공합니다.

### 2026-03-23: 동적 지식 기반 분석 엔진으로 전환
- **학습 프롬프트 고도화**: AI가 '야후 파이낸스'의 실제 데이터 필드(ROE, PER, 매출성장률 등)를 인식하여 검색 가능한 JSON 규칙을 생성하도록 `ai-learning.ts` 프롬프트를 수정했습니다.
- **분석 엔진 로직 변경**: 하드코딩된 '7-Step 텐배거' 비중을 낮추고, AI가 추출한 커스텀 JSON 규칙을 최우선으로 사용하여 기업을 스코어링하도록 `analysis-engine.ts`를 개편했습니다.
- **결과 UI 전면 개편**: 분석 결과 화면에서 하드코딩된 단계 대신 'AI 학습 로직 부합도'와 상세 규칙별 근거(`ruleScores`)를 전면에 배치하여 투명성을 높였습니다.

### 2026-03-23: 관리자 페이지 UI 가독성 개선
- **텍스트 대비 강화**: 관리자 레이아웃의 라이트 배경에서 잘 보이지 않던 흰색 계열 텍스트(`데이터 라이브러리`, `시스템 설정` 제목 및 `Drive 동기화` 버튼 등)를 어두운 색상(`text-gray-900`, `text-gray-700`)으로 수정하여 가독성을 높였습니다.
- **상태 배너 스타일 수정**: 학습 진행 중 표시되는 블루 테마 배너의 텍스트 색상을 조정하여 시인성을 확보했습니다.
...
- **분석 프로세스 최적화**:
  - 구글 드라이브 파일 처리 로직을 개선하여, 특정 파일 선택 시 전체 드라이브 스캔 없이 해당 파일 정보만 즉시 활용하도록 최적화했습니다.
  - Gemini API 호출 시 모델명 앞에 `models/` 접두사가 누락되어 발생하던 404 에러를 수정했습니다.
- **코드 정밀 클리닝**:
  - 분석 엔진(`runAnalysisEngine`) 시그니처와 호출부의 매개변수 불일치 버그를 수정했습니다.
  - 사용되지 않는 변수, Import(`fs`, `path`), 함수(`calculateRuleScore`, `calculateHistoricalVolatility` 등)를 전면 제거하여 코드 품질을 높였습니다.
- **서비스 가용성**: 소스 수정 사항을 Git에 즉시 반영(Push)하고, 개발 서버를 재시작하여 최신 상태를 유지했습니다.

### 2026-03-24 (관리자 설정 고도화: 뉴스 스캔 AI 모델 선택 및 멀티 프로바이더 지원)
- **뉴스 스캔 AI 모델 설정**: 관리자 시스템 설정 페이지에서 기업 분석 후 뉴스 스캔에 사용할 AI 모델을 직접 선택할 수 있는 기능을 추가했습니다. 선택 가능한 모델 목록은 원천 데이터 스캔용 모델과 동일하게 제공됩니다.
- **분석 엔진 멀티 프로바이더 지원**: `market-context.ts`의 뉴스 감성 분석 및 주가 예측 기능을 리팩토링하여 Gemini뿐만 아니라 OpenAI(GPT-4o 등)와 Anthropic(Claude) 모델을 모두 사용할 수 있도록 확장했습니다.
- **분석 진행 상태 시각화**: 기업 분석 진행 바에 현재 어떤 AI 모델을 사용하여 뉴스 및 정밀 분석을 수행 중인지 모델명을 표시하여 투명성을 높였습니다.
- **환경 변수 자동 연동**: `NEWS_SCAN_AI_MODEL` 환경 변수를 도입하여 관리자 UI에서 설정한 값이 분석 엔진에 즉시 반영되도록 구현했습니다.

### 2026-03-22 (백엔드 안정화 및 AI 엔진 고도화: 비용 최적화 & 로그 개선)
- **2단계 분석 아키텍처 도입 (Cost Optimization)**: 모든 종목에 AI를 적용하던 방식에서 **[1단계: 전체 스크리닝(No AI)] -> [2단계: 상위 20개 정밀 분석(AI)]** 구조로 전면 개편했습니다. 이를 통해 AI API 호출 비용을 90% 이상 획기적으로 절감하면서도 분석의 질은 유지했습니다.
- **백테스팅 시스템 구축 (Simulation)**: 분석 결과 카드 및 상세 화면에서 즉시 실행 가능한 '개별 기업 백테스트'와 직접 티커를 입력하여 분석하는 '매뉴얼 백테스트' 기능을 추가했습니다. Recharts를 활용한 주가 차트 및 MDD, 수익률 등 핵심 지표 시뮬레이션을 제공합니다.
- **ReferenceError 해결**: `market-context.ts`에서 정의되지 않은 `prompt` 변수를 참조하던 런타임 오류를 수정했습니다.
- **AI SDK 연동 방식 통일**: `@google/genai` 신규 SDK의 API 호출 방식을 시스템 내 다른 모듈(`ai-learning.ts`)과 동일하게 `client.models.generateContent` 구조로 통일하여 안정성을 확보했습니다.
- **로그 노이즈 제거**: `yahoo-finance2`의 스키마 검증 에러가 백엔드 로그를 가득 채우던 문제를 해결하기 위해 `setGlobalConfig` 및 호출 시 `validateResult: false`를 명시적으로 적용했습니다.

### 2026-03-08 (투자 분석 엔진 거대 고도화: 매크로, 감성, 예측, 백테스팅)
- **매크로 시장 환경 (Macro Context)**: VIX 지수, 미국 10년물 국채 금리, S&P 500 추세를 실시간으로 분석하여 전역 시장 위험도를 분석 로직에 반영했습니다.
- **뉴스 감성 분석 (Sentiment Analysis)**: Yahoo Finance의 실시간 뉴스를 Gemini Flash로 분석하여 개별 기업에 대한 시장의 긍정/부정 여론을 점수화(±1.0 보정)하고 요약 근거를 제공합니다.
- **AI 미래 성장 예측 (Predictive Modeling)**: 재무 지표와 매크로, 감성 데이터를 결합하여 6개월 후의 예상 목표가와 기대 수익률, 그리고 논리적 근거를 AI가 산출합니다.
- **과거 성과 검증 (Backtesting)**: AI가 선정한 기업에 대해 과거 1년치 주가 데이터를 바탕으로 실제 수익률과 S&P 500 대비 승률을 계산하여 전략의 유효성을 입증합니다.

### 2026-03-24 (AI 지능형 엔진 고도화: 멀티모달 & 전문가 합의 시스템)
- **데이터 계층 구조 및 동적 카테고리 (Admin Library)**: 관리자 페이지의 데이터 라이브러리에 구글 드라이브 폴더 구조를 반영한 트리 렌더링을 구현했습니다. 또한 'OTHER' 카테고리를 폐지하고 실제 파일 확장자별로 자동 탭을 생성하여 대량의 리서치 자료를 형식별/구조별로 동시에 탐색할 수 있도록 고도화했습니다.
- **데이터 계층 구조 (Folder Hierarchy)**: 구글 드라이브의 폴더 구조를 사이트 내에서도 그대로 유지하여 보여주는 재귀적 트리 렌더링 시스템을 구현했습니다.
- **멀티모달 시각 분석 (Multimodal Vision)**: MP4 파일의 시각적 프레임을 AI가 직접 분석하여 차트 및 지표를 추출하는 `visualEvidence` 로직을 구현했습니다. Gemini 1.5 Pro 모델을 활용해 영상 근거를 확보합니다.
- **업종 상대 가치 정교화 (Sector-Relative Calibration)**: `sector_relative`, `sector_percentile` 벤치마크 타입을 도입하여, 개별 기업의 지표를 단순히 절대치로 평가하지 않고 해당 업종 평균 및 상위 20% 수준과 대조하여 스코어링하는 지능형 엔진을 구축했습니다.
- **전략 합성 및 전문가 합의 (Expert Consensus)**: 여러 파일에서 상충하는 의견을 AI가 "전략 합성 담당자"로서 조율하고, 논리적 정합성에 따른 `consensusScore`를 산출하여 투자 신뢰도를 수치화했습니다.
- **대시보드 로직 에디터 (Interactive Logic Editor)**: 학습된 투자 로직을 사용자가 직접 수정(가중치 조절, 벤치마크 변경)할 수 있는 편집 모드를 구현하여 'AI-인간 협업' 인터페이스를 완성했습니다.

### 2026-03-16 (투자 논리 추출 및 분석 엔진 고도화)
- **동적 투자 전략 추출**: 원천 자료를 분석하여 AI가 직접 투자 카테고리를 명명하고 논리를 설계하는 '제로-베이스' 전략 분석 프롬프트를 도입했습니다.
- **계량화 및 수치 대조**: 추출된 논리를 revenue_growth, roe 등 시스템 지표와 연동하기 위해 target_metric, condition, benchmark를 포함한 JSON 구조로 고도화했습니다.
- **Hard-Gate(Critical) 로직**: 저자가 강조한 필수 조건(isCritical) 미충족 시 해당 기업을 분석 결과에서 즉시 제외하거나 경고를 표시하는 엄격한 필터링 로직을 구현했습니다.
- **UI 시각화**: 기업 분석 결과 카드 및 상세 페이지에 'CRITICAL FAIL' 배지와 필수 조건 미달 사유를 표시하여 분석 결과의 투명성을 높였습니다.

### 2026-03-15 (구글 드라이브 동기화 오류 수정)
- **키 파싱 로직 개선**: 서비스 계정 키의 Private Key 내 개행문자(\n) 처리 방식을 정교화하여 OpenSSL 디코더 오류(ERR_OSSL_UNSUPPORTED)를 해결했습니다.
- **환경 변수 유연성**: 드라이브 폴더 ID를 .env에서 관리할 수 있도록 개선했습니다.

### 2026-03-14 (로그 관리 체계 개선)
- **로그 폴더 분리**: backend와 frontend 로그를 별도 폴더에 저장하도록 구조를 개선했습니다.
- **기능 개발**: frontend 로그 수집을 위한 API(/api/logs)와 클라이언트 유틸리티를 추가했습니다.
- **로그 초기화**: 시스템 시작 시 기존 로그를 자동 정리하는 기능을 추가했습니다.
### 2026-03-13 (데이터 정밀화 및 상세 분석 피드백)
- **티커 정규화**: 비교 시 영문자만 추출하여 BF.B와 BF-B를 동일 종목으로 인식하도록 개선했습니다.
- **분석 제외 상세 조회**: 분석에서 제외된 종목 리스트와 구체적인 사유(거래정지, 시세부재 등)를 툴팁으로 실시간 확인할 수 있습니다.
- **지수 통계 및 제외 정보 상시 표시**: Russell 1000, Overlap 수치 및 분석 제외 종목 정보를 UI에 항상 표시하도록 개선했습니다.

- **티커 정규화**: 비교 시 영문자만 추출하여 BF.B와 BF-B를 동일 종목으로 인식하도록 개선했습니다.
- **분석 제외 상세 조회**: 분석에서 제외된 종목 리스트와 구체적인 사유(거래정지, 시세부재 등)를 툴팁으로 실시간 확인할 수 있습니다.
- **지수 통계 고도화**: Russell 1000, S&P 500, 그리고 중복(Overlap) 종목 수를 정확히 계산하여 UI에 상시 표시합니다.

### 2026-03-12 (데이터 투명성 강화: 제외 종목 추적)
- **분석 제외 종목 표시**: 시세 부재 등으로 분석에서 탈락한 기업 갯수를 실시간으로 추적하고 UI(배지)에 표시합니다.
- **데이터 무결성 가이드**: 시계열 데이터 누락 및 티커 인식 불가 상황에 대한 필터링 기준을 정립했습니다.
- **분석 엔진 안정화**: 백그라운드 작업 상태 관리에 제외 카운트를 통합하여 프로세스 투명성을 높였습니다.

### 2026-03-12 (기능 보완 및 로그/UI 개선)
- **Macrotrends 404 링크 해결**: 직접 검색 링크 대신 Google 검색 결과를 통해 근거 자료를 더 확실히 찾을 수 있도록 수정했습니다.
- **날짜별 폴더 로깅**: logs/YYYY-MM-DD/app.log 형태로 로그가 더 체계적으로 관리되도록 변경했습니다.
- **유니버스 카운트 표시**: 러셀1000과 S&P 500 종목 수 및 최종 분석 대상 수를 화면에 실시간으로 표시합니다.
- **분석 대상 수 차이 분석**: 야후 파이낸스 시세가 없거나 상장 폐지된 종목 등을 필터링하는 과정을 확인했습니다.

### 2026-03-12 (기능 고도화 및 최적화)
- **투자 기간(periodMonths) 전면 제거**: 분석 조건에서 투자 기간을 제거하고 7-step 규칙 자체에 집중하도록 수정했습니다.
- **Yahoo Finance 노이즈 제거**: 설문 알림 및 스키마 검증 오류 로그를 비활성화했습니다.
- **S&P 500 제외 로직 보완**: Wikipedia 파싱 정규식을 개선하여 NVDA 등 주요 종목이 누락되지 않도록 했습니다.


- **뉴스 조회 기능 주석 처리**: 개별 기업 카드 클릭 시 또는 분석 완료 후 실행되던 뉴스 조회 및 AI 번역 기능을 주석 처리하여 토큰 소모를 원천 차단했습니다. (기능 복구를 위해 소스코드는 유지)
...
- **`any` 타입 전면 제거**: `proxy.ts`, `auth.ts`, `auth.config.ts`, `stock.service.ts`, `data-library/page.tsx`, `plan/route.ts` 등 6개 파일에서 `as any`, `any` 파라미터를 proper 타입(NextAuth Session/JWT/User 확장, 커스텀 인터페이스)으로 교체.
  ...
  - `AnalysisJobStatus`, `KnowledgeContentShape`, `KnowledgeContent`, `AIModel`, `APIKeys` 등 인터페이스 정의
- **보안 강화**: 하드코딩 관리자 이메일(`pluss2.jh@gmail.com`) → `process.env.ADMIN_EMAIL` 환경변수화, `debug: true` → `process.env.NODE_ENV === 'development'` 조건부 전환, `.env.example` 신규 생성 (키 이름만 포함).
- **비동기 패턴 통일**: `.then().catch()` 체인 → `async/await` + `try/catch` 패턴으로 일관성 확보 (`stock.service.ts`, `data-library/page.tsx`).
- **매직 넘버 제거**: 폴링 인터벌 `5000` → `const POLLING_INTERVAL_MS = 5000`.
- **한국어 통일**: 영어 주석/`console.error` 메시지 20건+ → 한국어로 변환 (comment-checker 규칙 `language: ko` 준수).
- **빈 catch 블록 제거**: `} catch (e) { }` → `} catch { /* 학습 상태 폴링 실패 무시 */ }`.
- **Plan 타입 정합성**: `src/types/index.ts`의 Plan 타입을 Prisma enum과 일치하도록 `"FREE"|"STANDARD"|"PREMIUM"|"MASTER"`로 수정.
- **빌드 검증 완료**: `npm run build` → 44 페이지 정상 생성, TypeScript 에러 0건.


### 2026-02-25 (온보딩 플로우 및 아키텍처 개선)
- **온보딩 강제화**: 로그인한 사용자가 닉네임이 없는 경우(미가입) 즉시 회원가입 화면으로 리다이렉트하고, 가입 완료 후 기업 분석 화면으로 자동 이동하도록 구현.
  - `src/lib/auth.ts`, `src/proxy.ts`, `src/app/(auth)/register/onboarding/page.tsx` 수정.
- **Service Layer 도입**: `src/lib/services/stock.service.ts` 신설하여 비즈니스 로직 격리. (이전 작업 완료 건 기록)
- **성능 최적화**: 데이터 라이브러리 입력 지연 해결 및 소셜 로그인 설정 보강. (이전 작업 완료 건 기록)

### 2026-02-25 (성능 최적화 및 기능 수정)
- **데이터 라이브러리 최적화**: Input 컴포넌트 분리 및 onBlur 상태 업데이트 적용으로 타이핑 지연 해결.
- **분석 엔진 섹터 로직 강화**: 실시간 섹터 정보 대조 및 정규화 로직 추가로 필터링 정확도 향상.
- **학습 상태 동기화**: 서버 폴링을 통한 학습 중 UI 유지 및 강제 중지 API 연동.
- **소셜 로그인 복구**: auth.config.ts의 네이버 state 체크 추가 및 연동 허용 설정으로 Configuration 에러 해결.

### 2026-02-25 (아키텍처 개선 - 서비스 레이어 분리)
- **Service Layer 도입**: `src/lib/services/stock.service.ts` 신설. 백엔드 비즈니스 로직(주식 분석, AI 학습, 사용량 제한 체크)을 API 핸들러에서 서비스 레이어로 격리.
- **API Route 리팩토링**: `api/analysis`, `api/gdrive/learn` 엔드포인트가 `StockService`를 호출하도록 수정하여 결합도 낮춤.
- **계층화 아키텍처 확립**: UI-API-Service-Data 레이어 구분을 명확히 하여 유지보수성 및 확장성(백엔드 물리 분리 가능성) 확보.

# Project Context History

### 2026-02-25 (Turbopack CSS import 에러 해결)
- **`globals.css` @import 인라인 대체**: Turbopack이 `shadcn/tailwind.css`의 package.json `"style"` exports condition을 처리 못해 panic 발생하던 문제 해결.
  - `@import "shadcn/tailwind.css"` 줄 제거 후 `node_modules/shadcn/dist/tailwind.css` 전체 내용(keyframes, custom-variants, utility 95줄)을 `globals.css`에 직접 인라인 삽입.
  - `.next` 캐시 삭제 후 `npm run dev` 재실행 → Turbopack panic 없이 1.6초 Ready 확인.


# Project Context History

### 2026-02-25 (이어서 - 빌드 에러 해결)
- **tsconfig.json `scripts/` 제외**: `exclude` 배열에 `"scripts"` 추가 → scripts/ 내 `.ts` 파일이 Next.js TypeScript 빌드 검사에 포함되어 발생하던 implicit any 에러 해결.
- **`src/app/api/admin/users/route.ts` 타입 수정**: `admins.map(a => a.email)` 에서 implicit any 에러 → `(a: { email: string })` 명시적 타입으로 해결.
- **`npx prisma generate` 실행**: PrismaClient 미초기화 런타임 에러 해결 → 빌드 성공 확인.


# Project Context History

### 2026-02-25
- **소스 구조 재설계 및 정리**: 미사용 파일 12개 삭제, 빈 디렉토리 4개 제거 (기능 동작 무영향).
  - 삭제된 디렉토리: `src/app/(dashboard)/`, `src/app/(marketing)/`, `src/lib/email/`, `src/lib/stripe/`
  - 삭제된 파일: `src/lib/permissions.ts`, `src/hooks/use-mounted.ts`, `src/hooks/use-portone.ts`, `src/lib/stock-analysis/filtering-pipeline.ts`, `src/lib/stock-analysis/scoring.ts`, `src/lib/stock-analysis/evidence-chain.ts`
- **stock-analysis/index.ts exports 정리**: 삭제된 모듈 참조 제거, 실제 사용 모듈만 export.


# Project Context History

### 2026-02-23
- **auto-investment-dev 스킬 적용 완료**: 자동 푸시 및 로그 기록.
- **분석 상태 초기화 (로그아웃 연동)**: 로그아웃 시 백엔드의 `userAnalysisJobs`에서 해당 사용자의 진행 또는 완료된 투자 분석 기록을 모두 삭제(`DELETE /api/analysis`)하여 재로그인 시 초기화되도록 개선.
- **분석 검색 조건 추가 및 필터링 강화**: 
  - 기업 분석 시 '투자기간(1, 3, 6, 12개월)', '섹터(테크, 헬스케어, 에너지 등)', '투자 스타일(성장주/가치주)' 등급 선택 UI 구현.
  - 야후 파이낸스 실시간 데이터의 `assetProfile` 모듈을 연동.
  - 선택한 스타일에 부합할 경우 모델 스코어링 추가 부여(전략 부스팅) 로직을 분석 엔진(`analysis-engine.ts`)에 구현.

### 2026-02-22
- **auto-investment-dev 스킬 적용**: 자동화 규정에 따라 테스트 및 배포를 백그라운드에서 진행하고 Git 자동 푸시를 연동.
- **Drive 동기화 수동화**: 대시보드 및 라이브러리 접근 시 즉시 실행되던 구글 드라이브 동기화 기능을 제거하고, 명시적인 버튼 클릭 시에만 구글 API를 호출하도록 워크플로 변경.
- **UI 렌더링 성능 최적화 (Polling 개선)**: 전역 레이아웃에서 5초마다 호출되어 UI 스로틀링을 유발하던 `LearningStatusBadge` 컴포넌트의 상태 확인 주기를 30초로 완화.
- **소셜 로그인 다중 계정 취급 및 인증 아키텍처 개선**: 기존 이메일 접두어를 해킹해 분리하던 방식을 완전히 수정.
  - DB `User` 모델에 `provider`, `providerAccountId`, `role` 컬럼을 신설하고 복합 유니크 키(`@@unique`)를 적용해 같은 이메일을 사용해도 다른 소셜 계정일 경우 완벽하게 별개의 유저로 분리되도록 정석적인 방식을 도입.
  - 관리자 계정 하드코딩 권한 주입을 철거하고, DB 내 해당 유저 정보(구글 Provider + pluss2.jh@gmail.com + role='ADMIN') 일치 여부에 따라 완벽히 제어하도록 인가 콜백(`auth.ts` jwt/signIn) 전면 수정.
  - 위 변화에 맞춰 기존 관리자가 아닌 일반/테스트 유저 레코드 전체를 Prisma를 통해 일괄 초기화(삭제) 처리.
- **로그인 페이지 UI 정리**: 불필요한 "주식 분석 화면으로 돌아가기" 텍스트 링크와 하드코딩되었던 "관리자 로그인" 이메일/비밀번호 입력 폼을 소스에서 완전히 제거.

### 2026-02-21
- **API 모델 동적 호출 적용**: `admin/data-library` 화면에서 하드코딩된 모델 리스트(`EXT_MODELS`)를 버리고, `api/admin/models` API를 신설하여 OpenAPI(`openai`) 및 Google API 통신을 통해 현재 API키로 사용 가능한 최신 모델 목록을 실시간으로 가져와 맵핑하도록 개선.
- **분석 대시보드 UI 컬러 대비 개선**: Analysis Output(기업 분석 결과) 화면 내 전략 텍스트 컬러가 매우 밝아 눈에 잘 띄지 않던 문제(`text-gray-300` on `bg-gray-50/80`)를 `text-gray-600`로 수정. 세부 증빙 요소 팝오버의 배경색과 대비 컬러 등도 어두운 톤의 영역에 맞게 자연스럽게 가독성을 개선.
- **사용자 관리 화면 개편**: 사용자 목록을 단순 나열형의 큰 카드 디자인에서 콤팩트한 가로 리스트형으로 대폭 리디자인.
  - 상단에 '이름 또는 이메일'로 필터링할 수 있는 **검색(`Search`)** 기능을 추가.
  - `Plan` (FREE, STANDARD, PREMIUM, MASTER) 유형별로 사용자를 모아볼 수 있는 **탭(`Tabs`)** 인터페이스 적용 및 해당 탭별 실 사용자 수 표시.
- **AI 학습 UI 개편**:
  - 원천 데이터 스캔 시 모델을 선택하지 않은 확장자가 있을 경우 학습 시작 버튼을 자동으로 비활성화(`disabled`) 하도록 유효성 검사 로직(`isModelSelectionComplete`) 반영.
  - 데이터 라이브러리 최초 접근 시 불필요한 파일 스캔을 생략하고, 사용자가 `DRIVE 동기화` 버튼을 직접 클릭한 이후에만 구글 드라이브 동기화 및 자료 조회를 수행하도록 최적화.
- **분석 대시보드 UI 재배치**: 시장 유니버스 분석 후 나타나는 엔터프라이즈 스캔 결과 영역(그리드 목록)이 좌측 추천 설정 폼과 나란히 우측에 표시되도록 Tailwind CSS의 Grid column(`col-span-8` 블록 통합) 레이아웃 이슈 해결.
- **글로벌 전역 학습 상태 표시 UI 추가**: 백그라운드 AI 학습 중에도 관리자 계정이 사이트 내의 다른 어떤 페이지에 접근해도 최상단에 "학습 중..." 배지가 표시되도록 클라이언트 폴링 기반 API(`learning-status`) 도입.
- **AI 멀티 모델 파싱 버그 수정**: 빈 모델명 fallback 처리 오류로 발생하던 잘못된 API 모델 버젼 매핑(`gemini-1.5-pro` 누락으로 인한 404 Not Found 에러) 문제 해결.
- **AI 학습 오류 해결**: Google Drive에서 PDF 파일 연동 중 발생하던 Buffer 변수 참조 오류 (`ERR_INVALID_ARG_VALUE`)와 이로 인한 빈 파일 에러("처리할 수 있는 파일이 없습니다") 해결.
- **학습 백그라운드 전환 및 기민한 팝업 안내**: AI 학습 동작을 백그라운드로 전환하여 다른 메뉴 이동이 가능하도록 개선, 완료 시 팝업 안내 로직 추가.
- **AI 모델 선택 기능 구체화**: 확장자 탭별로 기존 Gemini/GPT/Claude의 그룹 대신 16가지 세부 모델을 각각 나열하고, API 키 등록 여부에 따라 버튼을 비활성화하는 로직 반영.

### 2026-02-20
- **UI 가독성 대폭 개선**: 어두운 테마에서 식별이 어려웠던 글자들(설명, 라벨, 가이드 문구 등)을 더 밝은 톤(gray-200, gray-300)으로 일괄 수정하여 전문적이고 선명한 UI 구현.
- **분석 시스템 안정화**: Free 회원의 분석 횟수가 남았음에도 버튼이 비활성화되던 로직 결함을 수정. Prisma의 유효성 검사 로직을 강화하여 데이터 정합성 확보.
- **회원가입 정책 완화**: 필수 대소문자/숫자 포함 규칙을 선택적 조합으로 변경(영어/숫자 8자 이상). 사용자 편의성 증대.
- **사용자 경험(UX) 최적화**: 기업 분석 시 불필요하게 반복되던 비용 발생 경고(Confirm) 창 제거.
- **관리자 대시보드 고도화**: 대시보드 전체의 명도를 높이고 지표별 텍스트 시인성 확보.

### 2024-05-22 (기준일)
- **용어 통일 및 Schema 최적화**: 'membershipTier' 필드를 제거하고 'plan'으로 통합 완료. 'FREE', 'STANDARD', 'PREMIUM', 'MASTER' 등급 체계 확립.
- **온보딩 프로세스 구현**: 소셜 로그인(구글, 카카오) 신규 사용자를 위한 추가 정보 입력(닉네임, 비밀번호) 페이지 및 API 구축.
- **권한 및 제한 사항 조정**: 무료 플랜 주간 분석 횟수를 1회로 조정. 관리자(pluss2.jh@gmail.com) 전용 'MASTER' 플랜 및 관리자 대시보드 접근 권한 정교화.
- **결제 시스템 골격 마련**: 프론트엔드 결제 선택 로직 및 백엔드 결제 검증 API(Mock) 구현. 세션 즉시 업데이트 기능 추가.
- **UI/UX 개선**: 사용자 메뉴의 클릭 방해 요소 제거(z-index 보정), 유료 API 호출 전 비용 발생 안내 문구 추가.
- **보안 강화**: 관리자 계정을 지정된 이메일로 엄격히 제한하고, DB 내 이전 관리자 정보 정리 완료.

### 보안 권한 체계 강화 및 카카오 로그인 이슈 해결

#### 변경사항
1. **관리자 권한 체계 전면 개편 (소스코드 내 하드코딩 제거)**
   - `auth.ts`, `proxy.ts`, `user-menu.tsx` 등 모든 파일에서 관리자 이메일(`pluss2.jh@gmail.com`) 하드코딩을 제거했습니다.
   - `AdminUser` 테이블 및 `role === 'ADMIN'` 기반의 동적 권한 확인 체계로 전환했습니다.
   - 이제 `AdminUser` 테이블에 등록된 이메일은 자동으로 `ADMIN` 역할과 `MASTER` 플랜을 부여받습니다.

2. **로그인 리다이렉트 흐름 개선**
   - 관리자 계정으로 로그인 시 즉시 관리자 대시보드(`/admin/dashboard`)로 이동하도록 수정했습니다.
   - 일반 사용자의 경우 소셜 로그인 후 분석 페이지 또는 이전 페이지로 정상 이동합니다.

3. **카카오 로그인 안정화 (Configuration 에러 해결)**
   - `auth.config.ts`: KakaoProvider에 `checks: ["state"]` 옵션을 추가하여 PKCE 관련 호환성 이슈를 방지했습니다.
   - `.env`: `AUTH_URL`, `NEXTAUTH_URL`, `AUTH_TRUST_HOST` 등 필수 환경 변수 설정을 명시화했습니다.

4. **사용자 메뉴 및 플랜 표시 정확도 향상**
   - 일반 사용자 계정 클릭 시 관리자 메뉴가 노출되지 않도록 필터링했습니다.
   - 우측 상단 사용자 메뉴에 현재 활성화된 올바른 플랜(Premium, MASTER 등)이 컬러 배지와 함께 정확히 표시되도록 수정했습니다.
   - `MASTER` 등급(관리자)의 경우 모든 유료 기능 제한을 자동으로 해제하는 로직을 강화했습니다.

5. **API 보안 강화**
   - 모든 `/api/admin/*` 엔드포인트에서 이메일 기반이 아닌 DB 역할 기반의 권한 검증을 수행하도록 일괄 수정했습니다.
...
- `.env` - GEMINI_API_KEY, GOOGLE_SERVICE_ACCOUNT_KEY 추가
